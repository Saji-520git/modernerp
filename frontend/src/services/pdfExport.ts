...
export async function exportSaleInvoice(sale: Sale): Promise<void> {
  ...
  const startY = drawPageHeader(
    ...
    [
      `Bill To: ${fullSale.customer?.name ?? 'Walk-in Customer'}`,
      `Warehouse: ${fullSale.warehouse.name} (${fullSale.warehouse.code})`,
      `Prepared by: ${fullSale.createdBy.fullName}`,
      ...(fullSale.note ? [`Note: ${fullSale.note}`] : []),
    ],
    [
      { label: 'STATUS', value: fullSale.status },
      { label: 'PAYMENT', value: paymentMethods[fullSale.paymentMethod] ?? fullSale.paymentMethod },
    ],
  );
  ...
  const totalsRows: { label: string; value: string; bold?: boolean }[] = [
    { label: 'Subtotal', value: formatMoney(fullSale.subtotalCents) },
    { label: 'Tax', value: formatMoney(fullSale.taxCents) },
    ...(fullSale.discountCents > 0
      ? [{ label: 'Discount', value: `− ${formatMoney(fullSale.discountCents)}` }]
      : []),
    { label: 'TOTAL', value: formatMoney(fullSale.totalCents), bold: true },
    ...(fullSale.status === 'CONFIRMED'
      ? [
          { label: `Paid (${paymentMethods[fullSale.paymentMethod] ?? ''})`, value: formatMoney(fullSale.paidCents) },
          {
